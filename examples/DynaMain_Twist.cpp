...
void setup() {
  ...
  const char* pub_topic_xml = "<dds>"
                  "<topic>"
                    "<name>rt/gr/status</name>"                           // REPLACE "GR" WITH YOUR NAME
  ...
  const char* sub_topic_xml = "<dds>"
                  "<topic>"
                    "<name>rt/gr/cmd_vel</name>"                           // REPLACE "GR" WITH YOUR NAME
  ...
  uint8_t status[numof(requests)];
  if (!uxr_run_session_until_all_status(&session, 1000, requests, status, numof(requests)))
  {
...
void on_topic(uxrSession* session, uxrObjectId object_id, uint16_t request_id, uxrStreamId stream_id, struct ucdrBuffer* mb, void* args) {
  ...
  Serial.println(status.data); // Print the message to the usb serial port for debugging.
	...
	DM_set_angle(0, twist.linear.x);
	DM_set_angle(1, twist.angular.z);

//  static char msg[20];
//	sprintf(msg, "Angle = %d", pos);
//	Serial.println(msg);
